...
export default function useCytoscapeGraph({
  ...
  const applyWaypointVisibility = useCallback((cy, withFit = true) => {
    ...
    if (!hideWaypointsRef.current) {
      cy.edges().forEach((edge) => {
        edge.style(
            'display', edge.data('nbWaypoints') > 0 ? 'none' : 'element');
      });
  ...
  useEffect(
      () => {
        ...
        const onRightClick = (event) => {
          ...
          const {x, y} =
              computeMenuPosition(containerRect, event.renderedPosition);
      ...
      [
        elements, setMenu, hideMenu, applyWaypointVisibility,
        setSelectedElementInfo
      ]);
  ...
  return {containerRef, cyRef, runLayout, fitGraph, showWayPoints};
}
